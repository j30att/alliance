...
@Component({
  selector: "app-tachograph-verification",
  templateUrl: "./tachograph-verification.component.html",
  styleUrls: ["./tachograph-verification.component.scss"],
})
export class TachographVerificationComponent
  extends RequestBase
  implements OnInit
{
  requestBody: any = {};
  ...
  async fillFormsFields(request: Request): Promise<void> {
    ...
    this.requestBody.dateOfBirth = !_.isEmpty(this.requestBody.dateOfBirth)
      ? new Date(this.requestBody.dateOfBirth)
      : "";
  }
  ...
  async onCreateButton(bodyForm: NgForm) {
    ...
    request.type = RequestType.TYPE_METROLOGICAL_VERIFY;
